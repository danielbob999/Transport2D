...
int GroundBodyComponent::getGroundMode() {
	return m_groundMode;
}

void GroundBodyComponent::setGroundMode(int mode) {
	m_groundMode = mode;
}
...
void GroundBodyComponent::generate() {
	...
	if (m_ground1Body != nullptr) {
		PhysicsSystem::getInstance()->getWorld()->DestroyBody(m_ground1Body);
	}
